...
class MasaTwitterSearchAgent(MeshAgent):
    def __init__(self):
        ...
        self.headers = {
            'Authorization': f'Bearer {os.getenv("MASA_API_KEY")}',
        ...
        self.metadata.update({
            ...
            'outputs': [
                ...
                {
                    'name': 'data',
                    'description': 'Structured Twitter search results data.',
                    'type': 'dict'
                }
            ],
            'external_apis': ['Masa'],
    ...
    async def search_twitter(self, query: str, max_results: int = 100) -> dict:
        try:
            ...
            response = requests.post(
                f"{self.api_url}/search/live/twitter",
            ...
            for attempt in range(max_attempts):
                status_response = requests.get(
                    f"{self.api_url}/search/live/twitter/status/{uuid}",
                    headers=self.headers
                )
            ...
            result_response = requests.get(
                f"{self.api_url}/search/live/twitter/result/{uuid}",
                headers=self.headers
            )
    ...
    def format_twitter_results(self, data: List) -> Dict:
        ...
        formatted_results = {
            "search_stats": {
                "total_results": len(valid_tweets),
                "has_results": len(valid_tweets) > 0
            },
            "tweets": []
        }
