...
class CoreAgent(BaseAgent):
    ...
    async def handle_message(
        ...
        **kwargs
        ...
        if kwargs.get("force_cot", False) or self._should_use_cot(message):
        ...
        final_response = await self.post_process(response, **kwargs)
        
        return final_response, image_url, tool_result
    
    async def pre_process(self, message: str, skip_pre_validation: bool = False, source_interface: str = None, **kwargs) -> bool:
        ...
        do_pre_validation = (
            False if source_interface in 
            ["api", "twitter", "twitter_reply", "farcaster", "farcaster_reply", "telegram", "terminal"]
            else True
        )
        ...
        if do_pre_validation:
            return await self.validation_manager.validate(
                message, 
                agent_name=self.personality_provider.get_name()
            )
